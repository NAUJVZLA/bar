...
export default function InventarioPage() {
  ...
  const handleVaciarMovimientosClick = async () => {
    const confirm = window.confirm(
      '¿Estás seguro de que deseas eliminar permanentemente todo el historial de movimientos de esta sede?\n\nEsta acción no se puede deshacer y borrará los registros de Supabase.'
    );
    if (confirm) {
      setErrorMsg('');
      setSuccessMsg('');
      try {
        await mockDb.vaciarMovimientos(activeSedeId);
        setSuccessMsg('Historial de movimientos vaciado correctamente.');
        setMovimientos([]);
      } catch (err: any) {
        setErrorMsg('Error al vaciar los movimientos: ' + (err.message || err));
      }
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex gap-2">
          {activeTab === 'insumos' ? (
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-3.5 h-3.5 flex-shrink-0 text-red-400">
                <path fillRule="evenodd" d="M8.75 1A2.75 2.75 0 006 3.75v.443c-.795.077-1.584.176-2.365.298a.75.75 0 10.23 1.482l.149-.022.842 8.14a2.75 2.75 0 002.738 2.466h5.812a2.75 2.75 0 002.738-2.466l.842-8.14.15.022a.75.75 0 10.23-1.482A41.802 41.802 0 0014 4.193V3.75A2.75 2.75 0 0011.25 1h-2.5zM10 4c.84 0 1.673.025 2.5.075V3.75c0-.69-.56-1.25-1.25-1.25h-2.5c-.69 0-1.25.56-1.25 1.25v.325C8.327 4.025 9.16 4 10 4zM8.58 7.72a.75.75 0 00-1.5.06l.3 7.5a.75.75 0 101.5-.06l-.3-7.5zm4.34.06a.75.75 0 10-1.5-.06l-.3 7.5a.75.75 0 101.5.06l.3-7.5z" clipRule="evenodd" />
              </svg>
              Vaciar Historial
            </button>
            */
